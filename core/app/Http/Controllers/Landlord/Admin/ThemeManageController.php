...
class ThemeManageController extends Controller
{
    ...
    public function all_theme()
    {
        ...
        // Check if foodie theme exists in database, if not add it
        $foodieTheme = Themes::where('slug', 'foodie')->first();
        if (!$foodieTheme) {
            Themes::create([
                'title' => 'Foodie - Food Products Theme',
                'description' => 'A modern theme designed specifically for food products, restaurants, and culinary businesses',
                'slug' => 'foodie',
                'status' => true,
                'unique_key' => 'foodie_theme_' . time(),
                'theme_url' => 'https://nazmart.com/themes/foodie'
            ]);
            $all_themes = Themes::orderBy('id', 'asc')->get();
        }
