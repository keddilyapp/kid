...
class PaymentGatewayCredential
{
    ...
    public static function exchange_rate_usd_to_egy(){
        $global_currency = get_static_option('site_global_currency');
        $egy_exchange_rate = get_static_option('site_' . strtolower($global_currency) . '_to_egy_exchange_rate');
        return empty($egy_exchange_rate) ? 30.92 : $egy_exchange_rate;
    }
    ...
    public static function get_paytabs_credential() : object
    {
        ...
        $paytabs->setCurrency('EGP'); // PayTabs requires Egyptian Pounds
        $paytabs->setEnv($test_mode);
        $paytabs->setExchangeRate(self::exchange_rate_usd_to_egy());
