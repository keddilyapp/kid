...
@if($global_currency != 'EGY' && $global_currency != 'EGP')
    <div class="form-group">
        <label for="site_{{strtolower($global_currency)}}_to_egy_exchange_rate">{{__($global_currency.' to EGY Exchange Rate')}}</label>
        <input type="text" class="form-control"
               name="site_{{strtolower($global_currency)}}_to_egy_exchange_rate"
               value="{{get_static_option('site_'.$global_currency.'_to_egy_exchange_rate')}}">
        <span class="info-text">{{__('enter '.$global_currency.' to EGY exchange rate. eg: 1'.$global_currency.' = ? EGY')}}</span>
    </div>
@endif